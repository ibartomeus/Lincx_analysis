...
Probabilities_all <- function(alpha,S) {
  survival <- rep(0,S)
  sims <- 500  ### number of simualtions to obtain the probabilities of species persistence. In the text we used 100,000
  for(y in 1:sims){
...
Probabilities_plant_pos <- function(alpha,S) {
  survival <- rep(0,S)
  sims <- 500  ### number of simualtions to obtain the probabilities of species persistence. In the text we used 100,000
  for(y in 1:sims){
    delta_t <- 0.01 #time step
    ...
    N0 <- rep(1,S) #initial conditions for species abundances
    N0 <- N0 / sum(N0)
    r <- sphere_sampling_plant_pos(S) ### sampling random K's in the unit sphere
    parms <- list(r=r, alpha = alpha) ##ODE
    model <- function(t,N,parms){ dN <- N * (parms$r - parms$alpha %*% N); list(dN)}
    sol <- ode(N0,time_step,model,parms)
    for(z in 1:S){
...
#now it work only positive values for plants.
sphere_sampling_plant_pos <- function(m) {
  r <- (rnorm(m))
  r[1:3] <- abs(r[1:3])
  d <- sqrt(sum(r^2))
  r <- r/d
